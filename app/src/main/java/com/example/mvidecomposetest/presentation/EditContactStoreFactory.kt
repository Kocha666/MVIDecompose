...
class EditContactStoreFactory() {
    ...
    fun create(contact: Contact): EditContactStore = object : EditContactStore,
        ...
        storeFactory.create(
            ...
            initialState = EditContactStore.State(username = contact.username, phone = contact.phone, id = contact.id),
            reducer = ReducerImpl,
            executorFactory = { ExecutorImpl() }
        ) {}.apply {
        Log.d("StoreFactory","Created EditContactStoreFactory " )
    }
    ...
    private inner class ExecutorImpl :
        ...
        override fun executeIntent(
            ...
            when (intent) {
                ...
                is EditContactStore.Intent.PhoneChange -> {
    ...
    private object ReducerImpl : Reducer<EditContactStore.State, Msg> {
        override fun EditContactStore.State.reduce(msg: Msg): EditContactStore.State {
            val oldState = this
            return when (msg) {
                is Msg.ChangePhone -> oldState.copy(phone = msg.phone)
                is Msg.ChangeUsername -> oldState.copy(username = msg.username)
            }
        }
